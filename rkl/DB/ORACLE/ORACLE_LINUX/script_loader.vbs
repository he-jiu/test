    ...
    Public Function execTask(file)	
        ...
        For Each strLine In strLines
            ...
            Else						' 要发送的脚本
                ...
                If(regTest("su - oracle", strLine) = 1) Then
                	Dim logoncmd
                    Do While(Len(logoncmd) = 0)
                        logoncmd = crt.Dialog.Prompt("输入数据库用户名（如：oracle):", "Logon", "", False)
                    Loop
                    currScreen.Send "su - "&logoncmd
                    nResult = currScreen.WaitForString(logoncmd, 1)
                    currScreen.Send Chr(13)
                    currScreen.WaitForString "!@#$%",2
                    Dim ispwd
                    ispwd = currScreen.Get(currScreen.CurrentRow,1,currScreen.CurrentRow,currScreen.CurrentColumn)
                    ispwd = Trim(ispwd)
                    If(Len(ispwd)>0) Then
                        If (Right(ispwd,Len(":")) = ":" Or Right(ispwd,Len("：")) = "：") Then
                            Dim strpwd
                            Do While(Len(strpwd) = 0)
		                        strpwd = crt.Dialog.Prompt("输入密码:", "Logon", "", True)
		                    Loop
                        ...
                        End If
                    End If
                    sid = crt.Dialog.Prompt("输入数据库SID，如不输入则使用默认SID:", "Logon", "", False)
                    If(Len(sid)>0) Then
                    	currScreen.Send "export ORACLE_SID=" & sid
                        currScreen.Send Chr(13)
                    End If
                    prompt = currScreen.Get(currScreen.CurrentRow,1,currScreen.CurrentRow,currScreen.CurrentColumn)
                    prompt = Trim(prompt)
                ...
                '登录数据库
                If(regTest("sqlplus ""/ as sysdba""", strLine) = 1) Then
                	Dim loginType, loginPrompt
                	loginPrompt = "请选择登录SQLPLUS方式：" & Chr(13) & Chr(10) & "1、执行sqlplus ""/ as sysdba""登录" & Chr(13) & Chr(10) & "2、依次执行sqlplus /nolog、conn / as sysdba登录" & Chr(13) & Chr(10) & "3、依次执行sqlplus、sqlplus ""/ as sysdba""登录" & Chr(13) & Chr(10) & "请输入1或2或3："
                	Do while(Len(loginType) = 0)
						loginType = crt.Dialog.Prompt(loginPrompt, "choose", "", False)
					Loop
					If (loginType = "1") Then
						currScreen.Send "sqlplus ""/ as sysdba"""
						currScreen.WaitForString "sqlplus ""/ as sysdba""", 2
                    	currScreen.Send Chr(13)
					ElseIf (loginType = "2") Then
						currScreen.Send "sqlplus /nolog"
						currScreen.WaitForString "sqlplus /nolog", 2
	                    currScreen.Send Chr(13)
	                    currScreen.Send "conn / as sysdba"
	                    currScreen.WaitForString "conn / as sysdba", 2
	                    currScreen.Send Chr(13)
					ElseIf (loginType = "3") Then
						currScreen.Send "sqlplus"
						currScreen.WaitForString "sqlplus", 2
	                    currScreen.Send Chr(13)
	                    currScreen.WaitForStrings "r-name:", "r-name：", 10
	                    currScreen.Send "sqlplus ""/ as sysdba"""
	                    currScreen.WaitForString "sqlplus ""/ as sysdba""", 2
	                    currScreen.Send Chr(13)
	                    currScreen.WaitForStrings "assword:", "assword：", 10
	                    currScreen.Send Chr(13)
					End If
	                currScreen.WaitForString "!@#$%>", 2
					prompt = currScreen.Get(currScreen.CurrentRow,1,currScreen.CurrentRow,currScreen.CurrentColumn)
                ...
                If(regTest("INSTALL_PATH", strLine) = 1) Then
                    Dim path
                    Do While(Len(path) = 0)
                        path = crt.Dialog.Prompt("输入数据库安装路径:", "path", "", False)
                    Loop
                    ...
                    strbase = Left(strbase, Len(strbase)-13)  
                    strbase = strbase & path 
                    strLine = ""
                    currScreen.Send	strbase
                    currScreen.WaitForString strbase, 2
                    currScreen.Send Chr(13)
                    currScreen.WaitForString "!@#$", 1
                    prompt = currScreen.Get(currScreen.CurrentRow,1,currScreen.CurrentRow,currScreen.CurrentColumn)
                    prompt = Trim(prompt)
                ...
                If Len(strLine)>0 Then
                    ...
                    If(regTest("bash", strLine) = 1) Then  
					 	currScreen.WaitForString "!@#$%",2
	                    prompt = currScreen.Get(currScreen.CurrentRow,1,currScreen.CurrentRow,currScreen.CurrentColumn)
	                    prompt = Trim(prompt)
                    End If
